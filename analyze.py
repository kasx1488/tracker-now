...
def save_master_to_monitor(addr: str, label: str = "", db_path=None) -> bool:
    """
    ...
    try:
        ...
        if existing:
            # Обновляем метку если она пустая
            if label:
                conn.execute(
        ...
        conn.execute(
            ...
            "(address, master, depth, parent, path_json, amount_received, first_seen) "
            "VALUES (?,?,0,NULL,'[]',0,?)",
            (addr, addr, ts)
        )
        conn.commit()
...
def helius_trace_funding_deep(deployer, launch_ts, max_depth=4):
    ...
    def _trace_level(target, target_ts, depth):
        ...
        try:
            ...
            if gmgn_ff and gmgn_ff not in SYSTEM_PROGRAMS:
                ...
                if gmgn_ff not in existing:
                    ...
                    print(f"  ↳ GMGN fund_from для {target[:20]}…: "
                          f"{gmgn_ff[:20]}… {gmgn_ff_sol:.3f} SOL{tag}")
                    funders.append({
                        "funder":     gmgn_ff,
        ...
        def _classify(finfo):
            ...
            # ПРОСЛОЙКА — два независимых критерия (любой достаточен):
            #   A) Мало транзакций вообще (< 15) — явно одноразовый
            #   B) Нет торговли И нет деплоев — "пустой" кошелёк сколько бы tx ни было
            #      (tx могут быть SOL-переводами между прослойками)
        ...
        # Рекурсия только для прослоек (и только если ещё не видели)
        for node in level_nodes:
            if node.get("is_proxy") and node["funder"] not in visited:
                # Временная точка для следующего уровня = когда прослойка получила SOL
    ...
    if token_creator and age and HELIUS_KEY and not _creator_mon:
        ...
        if not _chain:
            ...
            if _cex and not _masters:
                ...
                # Сохраняем в monitor.db как мастера
                _auto_label = (
                    f"auto-top:{ca[:8]}"
                    + (f" ({_tcr[0].get('symbol','?')})" if _tcr else "")
                )
                _is_new = save_master_to_monitor(_taddr, label=_auto_label)
                ...
                _path_str = (
                    ...
                    + "".join(
                        f" {p['funder'][:8]}… →"
                        for p in sorted(_proxies, key=lambda x: x.get("depth", 0))
                        if p["funder"] != _taddr
                    )
                    + f" [bold]{_taddr[:8]}…{_taddr[-4:]}[/bold]"
                )
                ...
                if _is_new:
                    c.print(
                        f"  [bold yellow]⚡ Добавлен в monitor.db — будет отслеживаться![/bold yellow]"
                    )
                    c.print(
                ...
                for _midx, _m in enumerate(_masters[:3], 1):
                    ...
                    # Строим path: прослойки → мастер
                    _path_parts = []
                    for _p in _proxies:
                        if _p.get("depth", 99) < _mdepth:
                            _path_parts.append(_p["funder"])
                    _path_parts.append(_maddr)

                    # Сохраняем в monitor.db
                    _auto_label = (
                        f"auto:{ca[:8]}"
                        + (f" ({_mcr[0].get('symbol','?')})" if _mcr else "")
                    )
                    _is_new = save_master_to_monitor(_maddr, label=_auto_label)
                    ...
                    _path_str = (
                        f"{token_creator[:8]}… →"
                        + "".join(f" {p[:8]}… →" for p in _path_parts[:-1])
                        + f" [bold]{_maddr[:8]}…{_maddr[-4:]}[/bold]"
                    )
                    ...
                    c.print(Panel(
                        ...
                        f"[bold]{_maddr}[/bold]\n"
                        f"[dim]Глубина: {_mdepth+1} уровень(ей) от деплоера[/dim]\n"
                        ...
                        border_style="green",
                        title="[bold green]🔑 МАСТЕР-КОШЕЛЁК НАЙДЕН[/bold green]",
                    ))

                    if _is_new:
                        c.print(
                            f"  [bold green]⚡ Мастер-кошелёк добавлен в monitor.db![/bold green]"
                        )
                        c.print(
                            f"  [dim]Запусти мониторинг: python3 monitor.py --watch[/dim]"
                        )
